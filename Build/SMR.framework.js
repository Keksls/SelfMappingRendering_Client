...
var PThread = {
 ...
 allocateUnusedWorker: function() {
  var worker;
  var pthreadMainJs = locateFile("build.worker.js");
...
function _GetJSLoadTimeInfo(loadTimePtr) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(3, 1, loadTimePtr);
 loadTimePtr = loadTimePtr >> 2;
 GROWABLE_HEAP_U32()[loadTimePtr] = Module.pageStartupTime || 0;
 GROWABLE_HEAP_U32()[loadTimePtr + 1] = Module.dataUrlLoadEndTime || 0;
 GROWABLE_HEAP_U32()[loadTimePtr + 2] = Module.codeDownloadTimeEnd || 0;
}

function _GetJSMemoryInfo(totalJSptr, usedJSptr) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(4, 1, totalJSptr, usedJSptr);
 totalJSptr = totalJSptr >> 3;
 usedJSptr = usedJSptr >> 3;
 if (performance.memory) {
  GROWABLE_HEAP_F64()[totalJSptr] = performance.memory.totalJSHeapSize;
  GROWABLE_HEAP_F64()[usedJSptr] = performance.memory.usedJSHeapSize;
 } else {
  GROWABLE_HEAP_F64()[totalJSptr] = NaN;
  GROWABLE_HEAP_F64()[usedJSptr] = NaN;
 }
}
...
function _JS_Accelerometer_IsRunning() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(5, 1);
...
function _JS_Accelerometer_Start(callback, frequency) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(6, 1, callback, frequency);
...
function _JS_Accelerometer_Stop() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(7, 1);
 if (JS_Accelerometer) {
...
function _JS_Cursor_SetImage(ptr, length) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(8, 1, ptr, length);
 ptr = ptr;
 var binary = "";
 for (var i = 0; i < length; i++) binary += String.fromCharCode(GROWABLE_HEAP_U8()[ptr + i]);
 Module.canvas.style.cursor = "url(data:image/cur;base64," + btoa(binary) + "),default";
}

function _JS_Cursor_SetShow(show) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(9, 0, show);
...
function _JS_DOM_MapViewportCoordinateToElementLocalCoordinate(viewportX, viewportY, targetX, targetY) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(10, 1, viewportX, viewportY, targetX, targetY);
 targetX = targetX >> 2;
 targetY = targetY >> 2;
 var canvas = document.querySelector(jsCanvasSelector());
 var rect = canvas && canvas.getBoundingClientRect();
 GROWABLE_HEAP_U32()[targetX] = viewportX - (rect ? rect.left : 0);
 GROWABLE_HEAP_U32()[targetY] = viewportY - (rect ? rect.top : 0);
}
...
function _JS_Eval_OpenURL(ptr) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(12, 1, ptr);
...
function _JS_FileSystem_Sync() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(13, 0);
...
function _JS_GetRandomBytes(destBuffer, numBytes) {
 ...
 var unshared = new Uint8Array(numBytes);
 crypto.getRandomValues(unshared);
 GROWABLE_HEAP_U8().set(unshared, destBuffer);
 return 1;
}
...
function _JS_LinearAccelerationSensor_Start(callback, frequency) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(17, 1, callback, frequency);
 JS_DefineAccelerometerMultiplier();
...
function _JS_GravitySensor_Start(callback, frequency) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(16, 1, callback, frequency);
...
function _JS_LinearAccelerationSensor_Stop() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(19, 1);
 if (JS_LinearAccelerationSensor) {
  if (typeof GravitySensor !== "undefined" || JS_GravitySensor_callback == 0) {
   JS_LinearAccelerationSensor.stop();
   JS_LinearAccelerationSensor.removeEventListener("reading", JS_LinearAccelerationSensor_eventHandler);
...
function _JS_Gyroscope_Start(callback, frequency) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(21, 1, callback, frequency);
...
function _JS_Gyroscope_Stop() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(22, 1);
 if (JS_Gyroscope) {
  JS_Gyroscope.stop();
  JS_Gyroscope.removeEventListener("reading", JS_Gyroscope_eventHandler);
  JS_Gyroscope = null;
...
function _JS_Init_CopyPaste() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(24, 1);
 var canvas = document.querySelector(jsCanvasSelector());
...
function _JS_MobileKeyboard_GetText(buffer, bufferSize) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(28, 1, buffer, bufferSize);
...
function _JS_MobileKeyboard_GetTextSelection(outStart, outLength) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(29, 1, outStart, outLength);
 outStart = outStart >> 2;
 outLength = outLength >> 2;
 if (!mobile_input) {
  GROWABLE_HEAP_I32()[outStart] = 0;
  GROWABLE_HEAP_I32()[outLength] = 0;
  return;
 }
 GROWABLE_HEAP_I32()[outStart] = mobile_input.input.selectionStart;
 GROWABLE_HEAP_I32()[outLength] = mobile_input.input.selectionEnd - mobile_input.input.selectionStart;
}

function _JS_MobileKeyboard_Hide(delay) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(30, 1, delay);
...
function _JS_MobileKeyboard_SetCharacterLimit(limit) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(31, 1, limit);
 if (!mobile_input) return;
 mobile_input.input.maxLength = limit;
}

function _JS_MobileKeyboard_SetText(text) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(32, 1, text);
 if (!mobile_input) return;
...
function _JS_MobileKeyboard_SetTextSelection(start, length) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(33, 1, start, length);
 if (!mobile_input) return;
...
function _JS_MobileKeyboard_Show(text, keyboardType, autocorrection, multiline, secure, alert, placeholder, characterLimit) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(34, 1, text, keyboardType, autocorrection, multiline, secure, alert, placeholder, characterLimit);
...
function _JS_Module_WebGLContextAttributes_PowerPreference() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(35, 1);
...
function _JS_OnScreenshotProgress(phase, progress) {
 try {
  if (typeof window.OnScreenshotProgress === "function") {
   window.OnScreenshotProgress(phase, progress);
  } else {}
 } catch (e) {
  console.error("JS_OnScreenshotProgress error:", e);
 }
}

function _JS_OnScreenshotReady(base64Ptr, w, h) {
 try {
  var base64 = UTF8ToString(base64Ptr);
  if (typeof window.OnScreenshotReady === "function") {
  ...
  console.error("JS_OnScreenshotReady error:", e);
 }
}
...
function _JS_OrientationSensor_Start(callback, frequency) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(39, 1, callback, frequency);
...
function _JS_ScreenOrientation_Lock(orientationLockType) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(43, 1, orientationLockType);
...
function _JS_SetMainLoop(func, fps, simulateInfiniteLoop) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(44, 1, func, fps, simulateInfiniteLoop);
 try {
...
function jsAudioCreateUncompressedSoundClip(buffer, error) {
 ...
 soundClip.getData = function(ptr, length) {
  ...
  var output = GROWABLE_HEAP_F32().subarray(startOutputBuffer, startOutputBuffer + (length >> 2));
...
function _JS_Sound_Create_Channel(callback, userData) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(45, 1, callback, userData);
...
function _JS_Sound_GetAudioBufferSampleRate(soundInstance) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(46, 1, soundInstance);
...
function _JS_Sound_GetLength(bufferInstance) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(47, 1, bufferInstance);
...
function _JS_Sound_GetLoadState(bufferInstance) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(48, 1, bufferInstance);
...
function _JS_Sound_GetMetaData(bufferInstance, metaData) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(49, 1, bufferInstance, metaData);
 metaData = metaData >> 2;
 if (WEBAudio.audioWebEnabled == 0) {
  GROWABLE_HEAP_U32()[metaData] = 0;
  GROWABLE_HEAP_U32()[metaData + 1] = 0;
  return false;
 }
 var soundClip = WEBAudio.audioInstances[bufferInstance];
 if (!soundClip) {
 ...
 GROWABLE_HEAP_U32()[metaData] = soundClip.getNumberOfChannels();
 GROWABLE_HEAP_U32()[metaData + 1] = soundClip.getFrequency();
 return true;
}
...
function _JS_Sound_Load(ptr, length, decompress, fmodSoundType) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(51, 1, ptr, length, decompress, fmodSoundType);
 if (WEBAudio.audioWebEnabled == 0) return 0;
 ptr = ptr;
 var audioData = new ArrayBuffer(length);
 new Uint8Array(audioData).set(GROWABLE_HEAP_U8().subarray(ptr, ptr + length));
...
function _JS_Sound_Load_PCM(channels, length, sampleRate, ptr) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(52, 1, channels, length, sampleRate, ptr);
 if (WEBAudio.audioWebEnabled == 0) return 0;
...
function _JS_Sound_Play(bufferInstance, channelInstance, offset, delay) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(53, 1, bufferInstance, channelInstance, offset, delay);
 if (WEBAudio.audioWebEnabled == 0) return;
...
function _JS_Sound_ReleaseInstance(instance) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(54, 0, instance);
...
function _JS_Sound_Set3D(channelInstance, spatialBlendLevel) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(56, 1, channelInstance, spatialBlendLevel);
 var channel = WEBAudio.audioInstances[channelInstance];
 channel.set3D(spatialBlendLevel);
}

function _JS_Sound_SetListenerOrientation(x, y, z, xUp, yUp, zUp) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(57, 1, x, y, z, xUp, yUp, zUp);
 if (WEBAudio.audioWebEnabled == 0) return;
 x = -x;
 y = -y;
 z = -z;
...
function _JS_Sound_SetListenerPosition(x, y, z) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(58, 1, x, y, z);
 if (WEBAudio.audioWebEnabled == 0) return;
 try {
  var l = WEBAudio.audioContext.listener;
  if (l.positionX) {
...
function _JS_Sound_SetLoop(channelInstance, loop) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(59, 1, channelInstance, loop);
 if (WEBAudio.audioWebEnabled == 0) return;
 var channel = WEBAudio.audioInstances[channelInstance];
...
function _JS_Sound_SetLoopPoints(channelInstance, loopStart, loopEnd) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(60, 1, channelInstance, loopStart, loopEnd);
 if (WEBAudio.audioWebEnabled == 0) return;
 var channel = WEBAudio.audioInstances[channelInstance];
...
function _JS_Sound_SetPaused(channelInstance, paused) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(61, 1, channelInstance, paused);
 if (WEBAudio.audioWebEnabled == 0) return;
 var channel = WEBAudio.audioInstances[channelInstance];
...
function _JS_Sound_SetPitch(channelInstance, v) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(62, 1, channelInstance, v);
 if (WEBAudio.audioWebEnabled == 0) return;
 try {
  var channel = WEBAudio.audioInstances[channelInstance];
  channel.setPitch(v);
...
function _JS_Sound_SetPosition(channelInstance, x, y, z) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(63, 1, channelInstance, x, y, z);
 if (WEBAudio.audioWebEnabled == 0) return;
 var channel = WEBAudio.audioInstances[channelInstance];
...
function _JS_Sound_SetVolume(channelInstance, v) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(64, 1, channelInstance, v);
 if (WEBAudio.audioWebEnabled == 0) return;
 try {
  var channel = WEBAudio.audioInstances[channelInstance];
  channel.setVolume(v);
...
function _JS_Sound_Stop(channelInstance, delay) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(65, 1, channelInstance, delay);
 if (WEBAudio.audioWebEnabled == 0) return;
 var channel = WEBAudio.audioInstances[channelInstance];
 channel.stop(delay);
}
...
function _JS_SystemInfo_GetBrowserName(buffer, bufferSize) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(67, 1, buffer, bufferSize);
...
function _JS_SystemInfo_GetCanvasClientSize(domElementSelector, outWidth, outHeight) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(68, 1, domElementSelector, outWidth, outHeight);
 ...
 GROWABLE_HEAP_F64()[outWidth] = w;
 GROWABLE_HEAP_F64()[outHeight] = h;
}

function _JS_SystemInfo_GetDocumentURL(buffer, bufferSize) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(69, 1, buffer, bufferSize);
...
function _JS_SystemInfo_GetGPUInfo(buffer, bufferSize) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(70, 1, buffer, bufferSize);
...
function _JS_SystemInfo_GetOS(buffer, bufferSize) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(72, 1, buffer, bufferSize);
...
function _JS_SystemInfo_GetScreenSize(outWidth, outHeight) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(74, 1, outWidth, outHeight);
 outWidth = outWidth >> 3;
 outHeight = outHeight >> 3;
 GROWABLE_HEAP_F64()[outWidth] = Module.SystemInfo.width;
 GROWABLE_HEAP_F64()[outHeight] = Module.SystemInfo.height;
}

function _JS_SystemInfo_GetStreamingAssetsURL(buffer, bufferSize) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(75, 1, buffer, bufferSize);
...
function _JS_Video_CanPlayFormat(format) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(83, 1, format);
...
function _JS_Video_Create(url) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(84, 1, url);
...
function jsVideoPlayPendingBlockedVideo(video) {
 jsVideoPendingBlockedVideos[video].play().then(function() {
  ...
  if (v.requestVideoFrameCallback) v.requestVideoFrameCallback(function() {
   v.isLoaded = true;
  });
 });
}
...
function _JS_Video_Destroy(video) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(85, 1, video);
...
function _JS_Video_Duration(video) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(86, 1, video);
 return videoInstances[video].duration;
}

function _JS_Video_EnableAudioTrack(video, trackIndex, enabled) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(87, 1, video, trackIndex, enabled);
 var v = videoInstances[video];
...
function _JS_Video_GetAudioLanguageCode(video, trackIndex, buffer, bufferSize) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(88, 1, video, trackIndex, buffer, bufferSize);
...
function _JS_Video_GetNumAudioTracks(video) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(89, 1, video);
 var tracks = videoInstances[video].audioTracks;
 return tracks ? tracks.length : 1;
}

function _JS_Video_GetPlaybackRate(video) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(90, 1, video);
 return videoInstances[video].playbackRate;
}

function _JS_Video_Height(video) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(91, 1, video);
 return videoInstances[video].videoHeight;
}

function _JS_Video_IsPlaying(video) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(92, 1, video);
 var v = videoInstances[video];
 return !v.paused && !v.ended;
}

function _JS_Video_IsReady(video) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(93, 1, video);
 var v = videoInstances[video];
...
function _JS_Video_IsSeeking(video) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(94, 1, video);
 var v = videoInstances[video];
 return v.seeking;
}

function _JS_Video_Pause(video) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(95, 1, video);
 var v = videoInstances[video];
 v.pause();
 jsVideoRemovePendingBlockedVideo(video);
 if (v.loopEndPollInterval) {
...
function _JS_Video_SetLoop(video, loop) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(97, 1, video, loop);
 var v = videoInstances[video];
 if (v.loopEndPollInterval) {
  clearInterval(v.loopEndPollInterval);
 }
...
function _JS_Video_Play(video, muted) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(96, 1, video, muted);
 var v = videoInstances[video];
...
function _JS_Video_Seek(video, time) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(98, 1, video, time);
 var v = videoInstances[video];
 v.lastSeenPlaybackTime = v.currentTime = time;
}

function _JS_Video_SetEndedHandler(video, ref, onended) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(99, 1, video, ref, onended);
 var v = videoInstances[video];
...
function _JS_Video_SetErrorHandler(video, ref, onerror) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(100, 1, video, ref, onerror);
...
function _JS_Video_SetMute(video, muted) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(101, 1, video, muted);
 var v = videoInstances[video];
 v.muted = muted || jsVideoAllAudioTracksAreDisabled(v);
}

function _JS_Video_SetPlaybackRate(video, rate) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(102, 1, video, rate);
 videoInstances[video].playbackRate = rate;
}

function _JS_Video_SetReadyHandler(video, ref, onready) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(103, 1, video, ref, onready);
...
function _JS_Video_SetSeekedHandler(video, ref, onseeked) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(104, 1, video, ref, onseeked);
...
function _JS_Video_SetVolume(video, volume) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(105, 1, video, volume);
 videoInstances[video].volume = volume;
}

function _JS_Video_Time(video) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(106, 1, video);
...
function _JS_Video_UpdateToTexture(video, tex, adjustToLinearspace) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(107, 1, video, tex, adjustToLinearspace);
 var v = videoInstances[video];
...
function _JS_Video_Width(video) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(108, 1, video);
...
function _JS_WebCamVideo_GetNativeHeight(deviceId) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(109, 1, deviceId);
 return activeWebCams[deviceId] && activeWebCams[deviceId].video.videoHeight;
}

function _JS_WebCamVideo_GetNativeWidth(deviceId) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(110, 1, deviceId);
...
function _JS_WebCamVideo_GrabFrame(deviceId, buffer, destWidth, destHeight) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(111, 1, deviceId, buffer, destWidth, destHeight);
 ...
 GROWABLE_HEAP_U8().set(context.getImageData(0, 0, destWidth, destHeight).data, buffer);
 return 1;
}
...
function _JS_WebRequest_Abort(requestId) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(113, 1, requestId);
...
function _JS_WebRequest_Create(url, method) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(114, 1, url, method);
...
function _JS_WebRequest_GetResponseMetaData(requestId, headerBuffer, headerSize, responseUrlBuffer, responseUrlSize) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(115, 1, requestId, headerBuffer, headerSize, responseUrlBuffer, responseUrlSize);
 var response = wr.responses[requestId];
 if (!response) {
...
function _JS_WebRequest_GetResponseMetaDataLengths(requestId, buffer) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(116, 1, requestId, buffer);
 buffer = buffer >> 2;
 var response = wr.responses[requestId];
 if (!response) {
  GROWABLE_HEAP_U32()[buffer] = 0;
  GROWABLE_HEAP_U32()[buffer + 1] = 0;
  return;
 }
 var headers = jsWebRequestGetResponseHeaderString(requestId);
 GROWABLE_HEAP_U32()[buffer] = lengthBytesUTF8(headers);
 GROWABLE_HEAP_U32()[buffer + 1] = lengthBytesUTF8(response.url);
}

function _JS_WebRequest_Release(requestId) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(117, 1, requestId);
...
function _JS_WebRequest_Send(requestId, ptr, length, arg, onresponse, onprogress) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(118, 1, requestId, ptr, length, arg, onresponse, onprogress);
 ptr = ptr;
 ...
 function HandleSuccess(response, body) {
  ...
  if (requestOptions.init.enableStreamingDownload) {
   ...
   GROWABLE_HEAP_U8().set(body, buffer);
 ...
 function HandleProgress(e) {
  ...
  if (e.chunk) {
   ...
   GROWABLE_HEAP_U8().set(e.chunk, buffer);
 ...
 try {
  if (length > 0) {
   var postData = GROWABLE_HEAP_U8().subarray(ptr, ptr + length);
...
function _JS_WebRequest_SetRedirectLimit(request, redirectLimit) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(119, 1, request, redirectLimit);
...
function _JS_WebRequest_SetRequestHeader(requestId, header, value) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(120, 1, requestId, header, value);
 var requestOptions = wr.requests[requestId];
 if (!requestOptions) {
  return;
 }
...
function _JS_WebRequest_SetTimeout(requestId, timeout) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(121, 1, requestId, timeout);
 var requestOptions = wr.requests[requestId];
 if (!requestOptions) {
  return;
 }
...
function ___emscripten_init_main_thread_js(tb) {
 __emscripten_thread_init(tb, !ENVIRONMENT_IS_WORKER, 1, !ENVIRONMENT_IS_WEB, 524288);
...
function pthreadCreateProxied(pthread_ptr, attr, startRoutine, arg) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(122, 1, pthread_ptr, attr, startRoutine, arg);
...
function ___syscall__newselect(nfds, readfds, writefds, exceptfds, timeout) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(123, 1, nfds, readfds, writefds, exceptfds, timeout);
 try {
...
function ___syscall_accept4(fd, addr, addrlen, flags, d1, d2) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(124, 1, fd, addr, addrlen, flags, d1, d2);
 try {
...
function ___syscall_bind(fd, addr, addrlen, d1, d2, d3) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(125, 1, fd, addr, addrlen, d1, d2, d3);
 try {
  var sock = getSocketFromFD(fd);
...
function ___syscall_chmod(path, mode) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(126, 1, path, mode);
 try {
...
function ___syscall_connect(fd, addr, addrlen, d1, d2, d3) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(127, 1, fd, addr, addrlen, d1, d2, d3);
 try {
  var sock = getSocketFromFD(fd);
  var info = getSocketAddress(addr, addrlen);
...
function ___syscall_dup3(fd, suggestFD, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(128, 1, fd, suggestFD, flags);
 try {
...
function ___syscall_faccessat(dirfd, path, amode, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(129, 1, dirfd, path, amode, flags);
 try {
  path = SYSCALLS.getStr(path);
  assert(flags === 0);
...
function ___syscall_fcntl64(fd, cmd, varargs) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(130, 1, fd, cmd, varargs);
...
function ___syscall_fstat64(fd, buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(131, 1, fd, buf);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function ___syscall_ftruncate64(fd, length) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(132, 1, fd, length);
 try {
...
function ___syscall_getcwd(buf, size) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(133, 1, buf, size);
 try {
  if (size === 0) return -28;
  var cwd = FS.cwd();
...
function ___syscall_getdents64(fd, dirp, count) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(134, 1, fd, dirp, count);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  if (!stream.getdents) {
...
function ___syscall_getpeername(fd, addr, addrlen, d1, d2, d3) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(135, 1, fd, addr, addrlen, d1, d2, d3);
 try {
  var sock = getSocketFromFD(fd);
  if (!sock.daddr) {
   return -53;
...
function ___syscall_getsockname(fd, addr, addrlen, d1, d2, d3) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(136, 1, fd, addr, addrlen, d1, d2, d3);
 try {
  var sock = getSocketFromFD(fd);
...
function ___syscall_getsockopt(fd, level, optname, optval, optlen, d1) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(137, 1, fd, level, optname, optval, optlen, d1);
 try {
  var sock = getSocketFromFD(fd);
  if (level === 1) {
   if (optname === 4) {
...
function ___syscall_ioctl(fd, op, varargs) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(138, 1, fd, op, varargs);
 SYSCALLS.varargs = varargs;
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  switch (op) {
...
function ___syscall_listen(fd, backlog) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(139, 1, fd, backlog);
 try {
  var sock = getSocketFromFD(fd);
...
function ___syscall_lstat64(path, buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(140, 1, path, buf);
 try {
  path = SYSCALLS.getStr(path);
...
function ___syscall_mkdirat(dirfd, path, mode) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(141, 1, dirfd, path, mode);
 try {
  path = SYSCALLS.getStr(path);
  path = SYSCALLS.calculateAt(dirfd, path);
  path = PATH.normalize(path);
...
function ___syscall_newfstatat(dirfd, path, buf, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(142, 1, dirfd, path, buf, flags);
 try {
  path = SYSCALLS.getStr(path);
...
function ___syscall_openat(dirfd, path, flags, varargs) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(143, 1, dirfd, path, flags, varargs);
 SYSCALLS.varargs = varargs;
 try {
  path = SYSCALLS.getStr(path);
  path = SYSCALLS.calculateAt(dirfd, path);
...
function ___syscall_pipe(fdPtr) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(144, 1, fdPtr);
 try {
  if (fdPtr == 0) {
...
function ___syscall_poll(fds, nfds, timeout) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(145, 1, fds, nfds, timeout);
 try {
  var nonzero = 0;
...
function ___syscall_readlinkat(dirfd, path, buf, bufsize) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(146, 1, dirfd, path, buf, bufsize);
 try {
  path = SYSCALLS.getStr(path);
  path = SYSCALLS.calculateAt(dirfd, path);
...
function ___syscall_recvfrom(fd, buf, len, flags, addr, addrlen) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(147, 1, fd, buf, len, flags, addr, addrlen);
 try {
  var sock = getSocketFromFD(fd);
...
function ___syscall_recvmsg(fd, message, flags, d1, d2, d3) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(148, 1, fd, message, flags, d1, d2, d3);
 try {
  var sock = getSocketFromFD(fd);
...
function ___syscall_renameat(olddirfd, oldpath, newdirfd, newpath) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(149, 1, olddirfd, oldpath, newdirfd, newpath);
 try {
...
function ___syscall_rmdir(path) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(150, 1, path);
 try {
  path = SYSCALLS.getStr(path);
  FS.rmdir(path);
  return 0;
...
function ___syscall_sendmsg(fd, message, flags, d1, d2, d3) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(151, 1, fd, message, flags, d1, d2, d3);
 try {
  var sock = getSocketFromFD(fd);
  var iov = GROWABLE_HEAP_U32()[message + 8 >> 2];
  var num = GROWABLE_HEAP_I32()[message + 12 >> 2];
...
function ___syscall_sendto(fd, message, length, flags, addr, addr_len) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(152, 1, fd, message, length, flags, addr, addr_len);
 try {
  var sock = getSocketFromFD(fd);
...
function ___syscall_socket(domain, type, protocol) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(153, 1, domain, type, protocol);
 try {
...
function ___syscall_stat64(path, buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(154, 1, path, buf);
 try {
  path = SYSCALLS.getStr(path);
...
function ___syscall_statfs64(path, size, buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(155, 1, path, size, buf);
 try {
  path = SYSCALLS.getStr(path);
  assert(size === 64);
...
function ___syscall_symlink(target, linkpath) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(156, 1, target, linkpath);
 try {
...
function ___syscall_truncate64(path, length) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(157, 1, path, length);
 try {
  length = bigintToI53Checked(length);
  if (isNaN(length)) return -61;
  path = SYSCALLS.getStr(path);
...
function ___syscall_unlinkat(dirfd, path, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(158, 1, dirfd, path, flags);
 try {
  path = SYSCALLS.getStr(path);
  path = SYSCALLS.calculateAt(dirfd, path);
  if (flags === 0) {
...
function ___syscall_utimensat(dirfd, path, times, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(159, 1, dirfd, path, times, flags);
 try {
  path = SYSCALLS.getStr(path);
  assert(flags === 0);
...
function __mmap_js(len, prot, flags, fd, off, allocated, addr) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(160, 1, len, prot, flags, fd, off, allocated, addr);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function __munmap_js(addr, len, prot, flags, fd, offset) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(161, 1, addr, len, prot, flags, fd, offset);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  if (prot & 2) {
...
function getCanvasSizeMainThread(target, width, height) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(163, 1, target, width, height);
...
function setCanvasElementSizeMainThread(target, width, height) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(164, 1, target, width, height);
...
function _emscripten_exit_fullscreen() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(162, 1);
...
function _emscripten_exit_pointerlock() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(165, 1);
...
function _emscripten_get_fullscreen_status(fullscreenStatus) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(166, 1, fullscreenStatus);
 if (!JSEvents.fullscreenEnabled()) return -1;
...
function _emscripten_get_gamepad_status(index, gamepadState) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(167, 1, index, gamepadState);
...
function _emscripten_request_fullscreen(target, deferUntilInEventHandler) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(169, 1, target, deferUntilInEventHandler);
 var strategy = {
  scaleMode: 0,
...
function _emscripten_request_pointerlock(target, deferUntilInEventHandler) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(170, 1, target, deferUntilInEventHandler);
 target = findEventTarget(target);
 if (!target) return -4;
 if (!target.requestPointerLock) {
  return -1;
...
function _emscripten_set_gamepadconnected_callback_on_thread(userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(175, 1, userData, useCapture, callbackfunc, targetThread);
 if (_emscripten_sample_gamepad_data()) return -1;
 return registerGamepadEventCallback(2, userData, useCapture, callbackfunc, 26, "gamepadconnected", targetThread);
}

function _emscripten_set_gamepaddisconnected_callback_on_thread(userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(176, 1, userData, useCapture, callbackfunc, targetThread);
 if (_emscripten_sample_gamepad_data()) return -1;
...
function _emscripten_set_keypress_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(178, 1, target, userData, useCapture, callbackfunc, targetThread);
...
function _emscripten_set_touchmove_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(186, 1, target, userData, useCapture, callbackfunc, targetThread);
 return registerTouchEventCallback(target, userData, useCapture, callbackfunc, 24, "touchmove", targetThread);
}

function _emscripten_set_touchstart_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(187, 1, target, userData, useCapture, callbackfunc, targetThread);
...
function _emscripten_set_wheel_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(188, 1, target, userData, useCapture, callbackfunc, targetThread);
 target = findEventTarget(target);
 if (!target) return -4;
...
function _environ_get(__environ, environ_buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(189, 1, __environ, environ_buf);
 var bufSize = 0;
...
function _environ_sizes_get(penviron_count, penviron_buf_size) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(190, 1, penviron_count, penviron_buf_size);
...
function _fd_close(fd) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(191, 1, fd);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  FS.close(stream);
  return 0;
...
function _fd_fdstat_get(fd, pbuf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(192, 1, fd, pbuf);
 try {
  var rightsBase = 0;
...
function _fd_read(fd, iov, iovcnt, pnum) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(193, 1, fd, iov, iovcnt, pnum);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function _fd_seek(fd, offset, whence, newOffset) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(194, 1, fd, offset, whence, newOffset);
 try {
...
function _fd_write(fd, iov, iovcnt, pnum) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(195, 1, fd, iov, iovcnt, pnum);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function _getaddrinfo(node, service, hint, out) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(196, 1, node, service, hint, out);
 var addrs = [];
 var canon = null;
 var addr = 0;
 var port = 0;
...
function _gethostbyaddr(addr, addrlen, type) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(197, 1, addr, addrlen, type);
 if (type !== 2) {
  setErrNo(5);
  return null;
 }
...
function _gethostbyname(name) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(198, 1, name);
...
function _glBufferData(target, size, data, usage) {
 if (GROWABLE_HEAP_U8().length <= 2147483648) {
  if (data && size) {
...
function _glClearBufferfv(buffer, drawbuffer, value) {
 if (GROWABLE_HEAP_U8().length <= 2147483648) {
  ...
  var view = GROWABLE_HEAP_F32().subarray(value >> 2, value + 16 >> 2);
...
function _glClearBufferuiv(buffer, drawbuffer, value) {
 if (GROWABLE_HEAP_U8().length <= 2147483648) {
  ...
  var view = GROWABLE_HEAP_U32().subarray(value >> 2, value + 16 >> 2);
...
function _glCompressedTexImage2D(target, level, internalFormat, width, height, border, imageSize, data) {
 ...
 } else if (GROWABLE_HEAP_U8().length <= 2147483648) {
...
function _glCompressedTexImage3D(target, level, internalFormat, width, height, depth, border, imageSize, data) {
 if (GLctx.currentPixelUnpackBufferBinding) {
  ...
  GLctx.compressedTexImage3D(target, level, internalFormat, width, height, depth, border, data ? GROWABLE_HEAP_U8().subarray(data, data + imageSize) : null);
 }
}
...
function _glCompressedTexSubImage3D(target, level, xoffset, yoffset, zoffset, width, height, depth, format, imageSize, data) {
 if (GLctx.currentPixelUnpackBufferBinding) {
  ...
  GLctx.compressedTexSubImage3D(target, level, xoffset, yoffset, zoffset, width, height, depth, format, data ? GROWABLE_HEAP_U8().subarray(data, data + imageSize) : null);
 }
}
...
function _glGetBufferSubData(target, offset, size, data) {
 ...
 if (GROWABLE_HEAP_U8().length <= 2147483648) {
  ...
  size && GLctx.getBufferSubData(target, offset, GROWABLE_HEAP_U8().subarray(data, data + size));
 }
}
...
function _glUniform1uiv(location, count, value) {
 if (count <= 288) {
  ...
  for (var i = 0; i < count; ++i) {
   view[i] = GROWABLE_HEAP_U32()[value + 4 * i >> 2];
  }
 } else {
  var view = GROWABLE_HEAP_U32().subarray(value >> 2, value + count * 4 >> 2);
 }
...
function _glUniform2uiv(location, count, value) {
 if (count <= 144) {
  ...
  for (var i = 0; i < 2 * count; i += 2) {
   view[i] = GROWABLE_HEAP_U32()[value + 4 * i >> 2];
   view[i + 1] = GROWABLE_HEAP_U32()[value + (4 * i + 4) >> 2];
  }
 } else {
  var view = GROWABLE_HEAP_U32().subarray(value >> 2, value + count * 8 >> 2);
 }
...
function _glUniform3uiv(location, count, value) {
 if (count <= 96) {
  ...
  for (var i = 0; i < 3 * count; i += 3) {
   view[i] = GROWABLE_HEAP_U32()[value + 4 * i >> 2];
   view[i + 1] = GROWABLE_HEAP_U32()[value + (4 * i + 4) >> 2];
   view[i + 2] = GROWABLE_HEAP_U32()[value + (4 * i + 8) >> 2];
  }
 } else {
  var view = GROWABLE_HEAP_U32().subarray(value >> 2, value + count * 12 >> 2);
 }
...
function _glUniform4uiv(location, count, value) {
 if (count <= 72) {
  ...
  for (var i = 0; i < 4 * count; i += 4) {
   ...
   view[i + 1] = GROWABLE_HEAP_U32()[value + (4 * i + 4) >> 2];
   view[i + 2] = GROWABLE_HEAP_U32()[value + (4 * i + 8) >> 2];
   view[i + 3] = GROWABLE_HEAP_U32()[value + (4 * i + 12) >> 2];
  }
 } else {
  var view = GROWABLE_HEAP_U32().subarray(value >> 2, value + count * 16 >> 2);
 }
...
function _glUnmapBuffer(target) {
 ...
 if (!(mapping.access & 16)) if (GL.currentContext.version >= 2 && GROWABLE_HEAP_U8().length <= 2147483648) {
...
function _glVertexAttrib4fv(index, v) {
 v = v >> 2;
 GLctx.vertexAttrib4f(index, GROWABLE_HEAP_F32()[v], GROWABLE_HEAP_F32()[v + 1], GROWABLE_HEAP_F32()[v + 2], GROWABLE_HEAP_F32()[v + 3]);
}
...
function _navigator_gpu_request_adapter_async(options, adapterCallback, userData) {
 ...
 options >>= 2;
 let gpu = navigator["gpu"], powerPreference = [ , "low-power", "high-performance" ][GROWABLE_HEAP_U32()[options]], opts = {};
 if (gpu) {
  if (options) {
   opts["forceFallbackAdapter"] = !!GROWABLE_HEAP_U32()[options + 1];
  ...
  gpu["requestAdapter"](opts).then(_wgpuMuteJsExceptions(cb)).catch(e => {
   console.error(`navigator.gpu.requestAdapter() Promise failed: ${e}`);
   cb();
  });
  return 1;
 }
...
function _wgpu_adapter_or_device_get_limits(adapterOrDevice, limits) {
 ...
 for (let limitName of _wgpu32BitLimitNames) {
  GROWABLE_HEAP_U32()[limits++] = l[limitName];
 }
}
...
function wgpuReadSupportedLimits(heap32Idx) {
 ...
 for (let limitName of _wgpu32BitLimitNames) {
  if (v = GROWABLE_HEAP_U32()[heap32Idx++]) requiredLimits[limitName] = v;
 }
 return requiredLimits;
}

function wgpuReadQueueDescriptor(heap32Idx) {
 return GROWABLE_HEAP_U32()[heap32Idx] ? {
  "label": utf8(GROWABLE_HEAP_U32()[heap32Idx])
 } : void 0;
}

function wgpuReadFeaturesBitfield(heap32Idx) {
 let requiredFeatures = [], v = GROWABLE_HEAP_U32()[heap32Idx];
...
function _wgpu_adapter_request_device_async(adapter, descriptor, deviceCallback, userData) {
 ...
 wgpu[adapter]["requestDevice"](desc).then(_wgpuMuteJsExceptions(cb)).catch(e => {
  console.error(`GPUAdapter.requestDevice() Promise failed: ${e}`);
  cb();
 });
}
...
function _wgpu_buffer_read_mapped_range(gpuBuffer, startOffset, subOffset, dst, size) {
 ...
 GROWABLE_HEAP_U8().set(new Uint8Array(wgpu[gpuBuffer].mappedRanges[startOffset], subOffset, size), wgpu_checked_shift(dst, 0));
}
...
function wgpuReadArrayOfWgpuObjects(ptr, numObjects) {
 ...
 assert(ptr != 0 || numObjects == 0, "assert(ptr != 0 || numObjects == 0) failed!");
 assert(ptr >> 2 << 2 == ptr);
 ptr >>= 2;
 var arrayOfObjects = new Array(numObjects);
 for (var i = 0; i < numObjects; ) {
  assert(GROWABLE_HEAP_U32()[ptr], "assert(HEAPU32[ptr]) failed!");
  assert(wgpu[GROWABLE_HEAP_U32()[ptr]], "assert(wgpu[HEAPU32[ptr]]) failed!");
  arrayOfObjects[i++] = wgpu[GROWABLE_HEAP_U32()[ptr++]];
 }
...
function _wgpu_canvas_context_configure(canvasContext, config) {
 ...
 config >>= 2;
 let desc = {
  "device": wgpu[GROWABLE_HEAP_U32()[config]],
  "format": GPUTextureAndVertexFormats[GROWABLE_HEAP_U32()[config + 1]],
  "usage": GROWABLE_HEAP_U32()[config + 2],
  "viewFormats": wgpuReadArrayOfWgpuObjects(GROWABLE_HEAP_U32()[config + 4], GROWABLE_HEAP_U32()[config + 3]),
  "colorSpace": HTMLPredefinedColorSpaces[GROWABLE_HEAP_U32()[config + 6]],
  "toneMapping": {
   "mode": [ , "standard", "extended" ][GROWABLE_HEAP_U32()[config + 7]]
  },
  "alphaMode": [ , "opaque", "premultiplied" ][GROWABLE_HEAP_U32()[config + 8]]
 };
...
function wgpuReadTimestampWrites(timestampWritesIndex) {
 let querySet = GROWABLE_HEAP_U32()[timestampWritesIndex];
 if (querySet) {
  let timestampWrites = {
   "querySet": wgpu[querySet]
  }, i;
  if ((i = GROWABLE_HEAP_I32()[timestampWritesIndex + 1]) >= 0) timestampWrites["beginningOfPassWriteIndex"] = i;
  if ((i = GROWABLE_HEAP_I32()[timestampWritesIndex + 2]) >= 0) timestampWrites["endOfPassWriteIndex"] = i;
...
function wgpuReadRenderPassDepthStencilAttachment(heap32Idx) {
 return GROWABLE_HEAP_U32()[heap32Idx] ? {
  "view": wgpu[GROWABLE_HEAP_U32()[heap32Idx]],
  "depthLoadOp": GPULoadOps[GROWABLE_HEAP_U32()[heap32Idx + 1]],
  "depthClearValue": GROWABLE_HEAP_F32()[heap32Idx + 2],
  "depthStoreOp": GPUStoreOps[GROWABLE_HEAP_U32()[heap32Idx + 3]],
  "depthReadOnly": !!GROWABLE_HEAP_U32()[heap32Idx + 4],
  "stencilLoadOp": GPULoadOps[GROWABLE_HEAP_U32()[heap32Idx + 5]],
  "stencilClearValue": GROWABLE_HEAP_U32()[heap32Idx + 6],
  "stencilStoreOp": GPUStoreOps[GROWABLE_HEAP_U32()[heap32Idx + 7]],
  "stencilReadOnly": !!GROWABLE_HEAP_U32()[heap32Idx + 8]
 } : void 0;
}
...
function _wgpu_command_encoder_begin_render_pass(commandEncoder, descriptor) {
 ...
 assert(descriptor >> 2 << 2 == descriptor);
 descriptor >>= 2;
 let colorAttachments = [], numColorAttachments = GROWABLE_HEAP_I32()[descriptor + 4], colorAttachmentsIdx = wgpu_checked_shift(GROWABLE_HEAP_U32()[descriptor + 2], 2), colorAttachmentsIdxDbl = colorAttachmentsIdx + 6 >> 1, maxDrawCount = GROWABLE_HEAP_F64()[descriptor >> 1], depthStencilAttachment = GROWABLE_HEAP_U32()[descriptor + 5];
 ...
 assert(depthStencilAttachment == 0 || wgpu[depthStencilAttachment] instanceof GPUTextureView, "assert(depthStencilAttachment == 0 || wgpu[depthStencilAttachment] instanceof GPUTextureView) failed!");
 assert(numColorAttachments >= 0, "assert(numColorAttachments >= 0) failed!");
 while (numColorAttachments--) {
  colorAttachments.push(GROWABLE_HEAP_U32()[colorAttachmentsIdx] ? {
   "view": wgpu[GROWABLE_HEAP_U32()[colorAttachmentsIdx]],
   "depthSlice": GROWABLE_HEAP_I32()[colorAttachmentsIdx + 1] < 0 ? void 0 : GROWABLE_HEAP_I32()[colorAttachmentsIdx + 1],
   "resolveTarget": wgpu[GROWABLE_HEAP_U32()[colorAttachmentsIdx + 2]],
   "storeOp": GPUStoreOps[GROWABLE_HEAP_U32()[colorAttachmentsIdx + 3]],
   "loadOp": GPULoadOps[GROWABLE_HEAP_U32()[colorAttachmentsIdx + 4]],
   "clearValue": [ GROWABLE_HEAP_F64()[colorAttachmentsIdxDbl], GROWABLE_HEAP_F64()[colorAttachmentsIdxDbl + 1], GROWABLE_HEAP_F64()[colorAttachmentsIdxDbl + 2], GROWABLE_HEAP_F64()[colorAttachmentsIdxDbl + 3] ]
  } : null);
 ...
 let desc = {
  ...
  "occlusionQuerySet": wgpu[GROWABLE_HEAP_U32()[descriptor + 14]],
...
function wgpuReadGpuImageCopyTexture(ptr) {
 ...
 ptr >>= 2;
 return {
  "texture": wgpu[GROWABLE_HEAP_U32()[ptr]],
  "mipLevel": GROWABLE_HEAP_I32()[ptr + 1],
  "origin": [ GROWABLE_HEAP_I32()[ptr + 2], GROWABLE_HEAP_I32()[ptr + 3], GROWABLE_HEAP_I32()[ptr + 4] ],
  "aspect": GPUTextureAspects[GROWABLE_HEAP_U32()[ptr + 5]]
 };
}
...
function wgpuReadGpuImageCopyBuffer(ptr) {
 ...
 return {
  ...
  "bytesPerRow": GROWABLE_HEAP_I32()[ptr + 2],
  "rowsPerImage": GROWABLE_HEAP_I32()[ptr + 3],
  "buffer": wgpu[GROWABLE_HEAP_U32()[ptr + 4]]
 };
}
...
function _wgpu_device_create_bind_group(device, layout, entries, numEntries) {
 ...
 let e = [];
 while (numEntries--) {
  let resource = wgpu[GROWABLE_HEAP_U32()[entries + 1]];
  assert(resource, "assert(resource) failed!");
  e.push({
   "binding": GROWABLE_HEAP_U32()[entries],
...
function wgpuReadBindGroupLayoutDescriptor(entries, numEntries) {
 ...
 while (numEntries--) {
  let entry = {
   "binding": GROWABLE_HEAP_U32()[entries],
   "visibility": GROWABLE_HEAP_U32()[entries + 1]
  }, type = GROWABLE_HEAP_U32()[entries + 2];
  entries += 4;
  assert(type >= 1 && type <= 5, "assert(type >= 1 && type <= 5) failed!");
  if (type == 1) {
   entry["buffer"] = {
    "type": GPUBufferBindingTypes[GROWABLE_HEAP_U32()[entries]],
    "hasDynamicOffset": !!GROWABLE_HEAP_U32()[entries + 1],
    "minBindingSize": wgpuReadI53FromU64HeapIdx(entries + 2)
   };
  } else if (type == 2) {
   entry["sampler"] = {
    "type": GPUSamplerBindingTypes[GROWABLE_HEAP_U32()[entries]]
   };
  } else if (type == 3) {
   entry["texture"] = {
    "sampleType": GPUTextureSampleTypes[GROWABLE_HEAP_U32()[entries]],
    "viewDimension": GPUTextureViewDimensions[GROWABLE_HEAP_U32()[entries + 1]],
    "multisampled": !!GROWABLE_HEAP_U32()[entries + 2]
   };
  } else if (type == 4) {
   entry["storageTexture"] = {
    "access": GPUStorageTextureSampleTypes[GROWABLE_HEAP_U32()[entries]],
    "format": GPUTextureAndVertexFormats[GROWABLE_HEAP_U32()[entries + 1]],
    "viewDimension": GPUTextureViewDimensions[GROWABLE_HEAP_U32()[entries + 2]]
   };
  } else {
...
function _wgpu_device_create_buffer(device, descriptor) {
 ...
 let desc = {
  ...
  "usage": GROWABLE_HEAP_U32()[descriptor + 2],
  "mappedAtCreation": !!GROWABLE_HEAP_U32()[descriptor + 3]
 };
...
function wgpuReadConstants(constants, numConstants) {
 ...
 let c = {};
 while (numConstants--) {
  c[utf8(GROWABLE_HEAP_U32()[constants + 3 >> 2])] = GROWABLE_HEAP_F64()[wgpu_checked_shift(constants + 8, 3)];
  constants += 16;
 }
 return c;
}
...
function wgpuReadGpuStencilFaceState(idx) {
 ...
 return {
  "compare": GPUCompareFunctions[GROWABLE_HEAP_U32()[idx]],
  "failOp": GPUStencilOperations[GROWABLE_HEAP_U32()[idx + 1]],
  "depthFailOp": GPUStencilOperations[GROWABLE_HEAP_U32()[idx + 2]],
  "passOp": GPUStencilOperations[GROWABLE_HEAP_U32()[idx + 3]]
 };
}
...
function wgpuReadGpuBlendComponent(idx) {
 assert(idx != 0, "assert(idx != 0) failed!");
 assert(GPUBlendOperations[GROWABLE_HEAP_U32()[idx]], "assert(GPUBlendOperations[HEAPU32[idx]]) failed!");
 assert(GPUBlendFactors[GROWABLE_HEAP_U32()[idx + 1]], "assert(GPUBlendFactors[HEAPU32[idx+1]]) failed!");
 assert(GPUBlendFactors[GROWABLE_HEAP_U32()[idx + 2]], "assert(GPUBlendFactors[HEAPU32[idx+2]]) failed!");
 return {
  "operation": GPUBlendOperations[GROWABLE_HEAP_U32()[idx]],
  "srcFactor": GPUBlendFactors[GROWABLE_HEAP_U32()[idx + 1]],
  "dstFactor": GPUBlendFactors[GROWABLE_HEAP_U32()[idx + 2]]
 };
}
...
function wgpuReadRenderPipelineDescriptor(descriptor) {
 ...
 assert(descriptor >> 2 << 2 == descriptor);
 descriptor >>= 2;
 let vertexBuffers = [], targets = [], vertexIdx = descriptor, numVertexBuffers = GROWABLE_HEAP_I32()[vertexIdx + 7], vertexBuffersIdx = wgpu_checked_shift(GROWABLE_HEAP_U32()[vertexIdx + 2], 2), primitiveIdx = vertexIdx + 10, depthStencilIdx = primitiveIdx + 5, multisampleIdx = depthStencilIdx + 17, fragmentIdx = multisampleIdx + 4, numTargets = GROWABLE_HEAP_I32()[fragmentIdx + 7], targetsIdx = wgpu_checked_shift(GROWABLE_HEAP_U32()[fragmentIdx + 2], 2), depthStencilFormat = GROWABLE_HEAP_U32()[depthStencilIdx], multisampleCount = GROWABLE_HEAP_U32()[multisampleIdx], fragmentModule = GROWABLE_HEAP_U32()[fragmentIdx + 6], pipelineLayoutId = GROWABLE_HEAP_U32()[fragmentIdx + 10], desc;
 ...
 while (numVertexBuffers--) {
  let attributes = [], numAttributes = GROWABLE_HEAP_I32()[vertexBuffersIdx + 2], attributesIdx = wgpu_checked_shift(GROWABLE_HEAP_U32()[vertexBuffersIdx], 2);
  assert(numAttributes >= 0, "assert(numAttributes >= 0) failed!");
  while (numAttributes--) {
   attributes.push({
    "offset": wgpuReadI53FromU64HeapIdx(attributesIdx),
    "shaderLocation": GROWABLE_HEAP_U32()[attributesIdx + 2],
    "format": GPUTextureAndVertexFormats[GROWABLE_HEAP_U32()[attributesIdx + 3]]
   });
   attributesIdx += 4;
  }
  vertexBuffers.push({
   "arrayStride": wgpuReadI53FromU64HeapIdx(vertexBuffersIdx + 4),
   "stepMode": [ , "vertex", "instance" ][GROWABLE_HEAP_U32()[vertexBuffersIdx + 3]],
 ...
 while (numTargets--) {
  targets.push(GROWABLE_HEAP_U32()[targetsIdx] ? {
   "format": GPUTextureAndVertexFormats[GROWABLE_HEAP_U32()[targetsIdx]],
   "blend": GROWABLE_HEAP_U32()[targetsIdx + 1] ? {
    "color": wgpuReadGpuBlendComponent(targetsIdx + 1),
    "alpha": wgpuReadGpuBlendComponent(targetsIdx + 4)
   } : void 0,
   "writeMask": GROWABLE_HEAP_U32()[targetsIdx + 7]
  } : null);
  targetsIdx += 8;
 }
 desc = {
  "vertex": {
   "module": wgpu[GROWABLE_HEAP_U32()[vertexIdx + 6]],
   "entryPoint": utf8(GROWABLE_HEAP_U32()[vertexIdx]) || void 0,
   "buffers": vertexBuffers,
   "constants": wgpuReadConstants(GROWABLE_HEAP_U32()[vertexIdx + 4], GROWABLE_HEAP_I32()[vertexIdx + 8])
  },
  "fragment": fragmentModule ? {
   "module": wgpu[fragmentModule],
   "entryPoint": utf8(GROWABLE_HEAP_U32()[fragmentIdx]) || void 0,
   "targets": targets,
   "constants": wgpuReadConstants(GROWABLE_HEAP_U32()[fragmentIdx + 4], GROWABLE_HEAP_I32()[fragmentIdx + 8])
  } : void 0,
  "primitive": {
   "topology": GPUPrimitiveTopologys[GROWABLE_HEAP_U32()[primitiveIdx]],
   "stripIndexFormat": GPUIndexFormats[GROWABLE_HEAP_U32()[primitiveIdx + 1]],
   "frontFace": [ , "ccw", "cw" ][GROWABLE_HEAP_U32()[primitiveIdx + 2]],
   "cullMode": [ , "none", "front", "back" ][GROWABLE_HEAP_U32()[primitiveIdx + 3]],
   "unclippedDepth": !!GROWABLE_HEAP_U32()[primitiveIdx + 4]
  },
  "depthStencil": depthStencilFormat ? {
   "format": GPUTextureAndVertexFormats[depthStencilFormat],
   "depthWriteEnabled": !!GROWABLE_HEAP_U32()[depthStencilIdx + 1],
   "depthCompare": GPUCompareFunctions[GROWABLE_HEAP_U32()[depthStencilIdx + 2]],
   "stencilReadMask": GROWABLE_HEAP_U32()[depthStencilIdx + 3],
   "stencilWriteMask": GROWABLE_HEAP_U32()[depthStencilIdx + 4],
   "depthBias": GROWABLE_HEAP_I32()[depthStencilIdx + 5],
   "depthBiasSlopeScale": GROWABLE_HEAP_F32()[depthStencilIdx + 6],
   "depthBiasClamp": GROWABLE_HEAP_F32()[depthStencilIdx + 7],
   "stencilFront": wgpuReadGpuStencilFaceState(depthStencilIdx + 8),
   "stencilBack": wgpuReadGpuStencilFaceState(depthStencilIdx + 12),
   "clampDepth": !!GROWABLE_HEAP_U32()[depthStencilIdx + 16]
  } : void 0,
  "multisample": multisampleCount ? {
   "count": multisampleCount,
   "mask": GROWABLE_HEAP_U32()[multisampleIdx + 1],
   "alphaToCoverageEnabled": !!GROWABLE_HEAP_U32()[multisampleIdx + 2]
  } : void 0,
...
function _wgpu_device_create_sampler(device, descriptor) {
 ...
 descriptor >>= 2;
 let desc = descriptor ? {
  "addressModeU": GPUAddressModes[GROWABLE_HEAP_U32()[descriptor]],
  "addressModeV": GPUAddressModes[GROWABLE_HEAP_U32()[descriptor + 1]],
  "addressModeW": GPUAddressModes[GROWABLE_HEAP_U32()[descriptor + 2]],
  "magFilter": GPUFilterModes[GROWABLE_HEAP_U32()[descriptor + 3]],
  "minFilter": GPUFilterModes[GROWABLE_HEAP_U32()[descriptor + 4]],
  "mipmapFilter": GPUMipmapFilterModes[GROWABLE_HEAP_U32()[descriptor + 5]],
  "lodMinClamp": GROWABLE_HEAP_F32()[descriptor + 6],
  "lodMaxClamp": GROWABLE_HEAP_F32()[descriptor + 7],
  "compare": GPUCompareFunctions[GROWABLE_HEAP_U32()[descriptor + 8]],
  "maxAnisotropy": GROWABLE_HEAP_U32()[descriptor + 9]
 } : void 0;
 return wgpuStoreAndSetParent(device["createSampler"](desc), device);
}

function wgpuReadShaderModuleCompilationHints(index) {
 let numHints = GROWABLE_HEAP_I32()[index + 2], hints = [], hintsIndex = wgpu_checked_shift(GROWABLE_HEAP_U32()[index], 2), layout;
 assert(numHints >= 0, "assert(numHints >= 0) failed!");
 while (numHints--) {
  layout = GROWABLE_HEAP_U32()[hintsIndex + 2];
  ...
  hints.push({
   "entryPoint": utf8(GROWABLE_HEAP_U32()[hintsIndex]),
...
function wgpuReadShaderModuleDescriptor(descriptor) {
 ...
 descriptor >>= 2;
 return {
  "code": utf8(GROWABLE_HEAP_U32()[descriptor]),
...
function _wgpu_device_create_texture(device, descriptor) {
 ...
 assert(descriptor >> 2 << 2 == descriptor);
 descriptor >>= 2;
 assert(GROWABLE_HEAP_U32()[descriptor + 8] >= 1, "assert(HEAPU32[descriptor+8] >= 1) failed!");
 assert(GROWABLE_HEAP_U32()[descriptor + 8] <= 3, "assert(HEAPU32[descriptor+8] <= 3) failed!");
 let desc = {
  "viewFormats": wgpuReadArrayOfWgpuObjects(GROWABLE_HEAP_U32()[descriptor], GROWABLE_HEAP_U32()[descriptor + 2]),
  "size": [ GROWABLE_HEAP_I32()[descriptor + 3], GROWABLE_HEAP_I32()[descriptor + 4], GROWABLE_HEAP_I32()[descriptor + 5] ],
  "mipLevelCount": GROWABLE_HEAP_I32()[descriptor + 6],
  "sampleCount": GROWABLE_HEAP_I32()[descriptor + 7],
  "dimension": GROWABLE_HEAP_U32()[descriptor + 8] + "d",
  "format": GPUTextureAndVertexFormats[GROWABLE_HEAP_U32()[descriptor + 9]],
  "usage": GROWABLE_HEAP_U32()[descriptor + 10]
 };
...
function _wgpu_encoder_set_bind_group(encoder, index, bindGroup, dynamicOffsets, numDynamicOffsets) {
 ...
 wgpu[encoder]["setBindGroup"](index, wgpu[bindGroup], GROWABLE_HEAP_U32(), wgpu_checked_shift(dynamicOffsets, 2), numDynamicOffsets);
}
...
function _wgpu_queue_submit_multiple_and_destroy(queue, commandBuffers, numCommandBuffers) {
 ...
 commandBuffers >>= 2;
 while (numCommandBuffers--) _wgpu_object_destroy(GROWABLE_HEAP_U32()[commandBuffers++]);
}
...
function _wgpu_queue_write_buffer(queue, buffer, bufferOffset, data, size) {
 ...
 assert(wgpu[buffer], "assert(wgpu[buffer]) failed!");
 assert(wgpu[buffer] instanceof GPUBuffer, "assert(wgpu[buffer] instanceof GPUBuffer) failed!");
 wgpu[queue]["writeBuffer"](wgpu[buffer], bufferOffset, GROWABLE_HEAP_U8(), wgpu_checked_shift(data, 0), size);
}
...
function _wgpu_queue_write_texture(queue, destination, data, bytesPerBlockRow, blockRowsPerImage, writeWidth, writeHeight, writeDepthOrArrayLayers) {
 ...
 assert(wgpu[queue] instanceof GPUQueue, "assert(wgpu[queue] instanceof GPUQueue) failed!");
 assert(destination, "assert(destination) failed!");
 wgpu[queue]["writeTexture"](wgpuReadGpuImageCopyTexture(destination), GROWABLE_HEAP_U8(), {
...
function _wgpu_texture_create_view(texture, descriptor) {
 ...
 descriptor >>= 2;
 let desc = descriptor ? {
  "format": GPUTextureAndVertexFormats[GROWABLE_HEAP_U32()[descriptor]],
  "dimension": GPUTextureViewDimensions[GROWABLE_HEAP_U32()[descriptor + 1]],
  "usage": GROWABLE_HEAP_U32()[descriptor + 2],
  "aspect": GPUTextureAspects[GROWABLE_HEAP_U32()[descriptor + 3]],
  "baseMipLevel": GROWABLE_HEAP_I32()[descriptor + 4],
  "mipLevelCount": GROWABLE_HEAP_I32()[descriptor + 5],
  "baseArrayLayer": GROWABLE_HEAP_I32()[descriptor + 6],
  "arrayLayerCount": GROWABLE_HEAP_I32()[descriptor + 7]
 } : void 0;
...
var proxiedFunctionTable = [ null, _proc_exit, exitOnMainThread, _GetJSLoadTimeInfo, _GetJSMemoryInfo, _JS_Accelerometer_IsRunning, _JS_Accelerometer_Start, _JS_Accelerometer_Stop, _JS_Cursor_SetImage, _JS_Cursor_SetShow, _JS_DOM_MapViewportCoordinateToElementLocalCoordinate, _JS_DOM_UnityCanvasSelector, _JS_Eval_OpenURL, _JS_FileSystem_Sync, _JS_Get_WASM_Size, _JS_GravitySensor_IsRunning, _JS_GravitySensor_Start, _JS_LinearAccelerationSensor_Start, _JS_GravitySensor_Stop, _JS_LinearAccelerationSensor_Stop, _JS_Gyroscope_IsRunning, _JS_Gyroscope_Start, _JS_Gyroscope_Stop, _JS_Init_ContextMenuHandler, _JS_Init_CopyPaste, _JS_LinearAccelerationSensor_IsRunning, _JS_MobileKeybard_GetIgnoreBlurEvent, _JS_MobileKeyboard_GetKeyboardStatus, _JS_MobileKeyboard_GetText, _JS_MobileKeyboard_GetTextSelection, _JS_MobileKeyboard_Hide, _JS_MobileKeyboard_SetCharacterLimit, _JS_MobileKeyboard_SetText, _JS_MobileKeyboard_SetTextSelection, _JS_MobileKeyboard_Show, _JS_Module_WebGLContextAttributes_PowerPreference, _JS_Module_WebGLContextAttributes_PremultipliedAlpha, _JS_Module_WebGLContextAttributes_PreserveDrawingBuffer, _JS_OrientationSensor_IsRunning, _JS_OrientationSensor_Start, _JS_OrientationSensor_Stop, _JS_Profiler_InjectJobs, _JS_RunQuitCallbacks, _JS_ScreenOrientation_Lock, _JS_SetMainLoop, _JS_Sound_Create_Channel, _JS_Sound_GetAudioBufferSampleRate, _JS_Sound_GetLength, _JS_Sound_GetLoadState, _JS_Sound_GetMetaData, _JS_Sound_Init, _JS_Sound_Load, _JS_Sound_Load_PCM, _JS_Sound_Play, _JS_Sound_ReleaseInstance, _JS_Sound_ResumeIfNeeded, _JS_Sound_Set3D, _JS_Sound_SetListenerOrientation, _JS_Sound_SetListenerPosition, _JS_Sound_SetLoop, _JS_Sound_SetLoopPoints, _JS_Sound_SetPaused, _JS_Sound_SetPitch, _JS_Sound_SetPosition, _JS_Sound_SetVolume, _JS_Sound_Stop, _JS_SystemInfo_GetAnimationFrameRate, _JS_SystemInfo_GetBrowserName, _JS_SystemInfo_GetCanvasClientSize, _JS_SystemInfo_GetDocumentURL, _JS_SystemInfo_GetGPUInfo, _JS_SystemInfo_GetMatchWebGLToCanvasSize, _JS_SystemInfo_GetOS, _JS_SystemInfo_GetPreferredDevicePixelRatio, _JS_SystemInfo_GetScreenSize, _JS_SystemInfo_GetStreamingAssetsURL, _JS_SystemInfo_HasAstcHdr, _JS_SystemInfo_HasCursorLock, _JS_SystemInfo_HasFullscreen, _JS_SystemInfo_HasWebGL, _JS_SystemInfo_HasWebGPU, _JS_SystemInfo_IsMobile, _JS_UnityEngineShouldQuit, _JS_Video_CanPlayFormat, _JS_Video_Create, _JS_Video_Destroy, _JS_Video_Duration, _JS_Video_EnableAudioTrack, _JS_Video_GetAudioLanguageCode, _JS_Video_GetNumAudioTracks, _JS_Video_GetPlaybackRate, _JS_Video_Height, _JS_Video_IsPlaying, _JS_Video_IsReady, _JS_Video_IsSeeking, _JS_Video_Pause, _JS_Video_Play, _JS_Video_SetLoop, _JS_Video_Seek, _JS_Video_SetEndedHandler, _JS_Video_SetErrorHandler, _JS_Video_SetMute, _JS_Video_SetPlaybackRate, _JS_Video_SetReadyHandler, _JS_Video_SetSeekedHandler, _JS_Video_SetVolume, _JS_Video_Time, _JS_Video_UpdateToTexture, _JS_Video_Width, _JS_WebCamVideo_GetNativeHeight, _JS_WebCamVideo_GetNativeWidth, _JS_WebCamVideo_GrabFrame, _JS_WebPlayer_FinishInitialization, _JS_WebRequest_Abort, _JS_WebRequest_Create, _JS_WebRequest_GetResponseMetaData, _JS_WebRequest_GetResponseMetaDataLengths, _JS_WebRequest_Release, _JS_WebRequest_Send, _JS_WebRequest_SetRedirectLimit, _JS_WebRequest_SetRequestHeader, _JS_WebRequest_SetTimeout, pthreadCreateProxied, ___syscall__newselect, ___syscall_accept4, ___syscall_bind, ___syscall_chmod, ___syscall_connect, ___syscall_dup3, ___syscall_faccessat, ___syscall_fcntl64, ___syscall_fstat64, ___syscall_ftruncate64, ___syscall_getcwd, ___syscall_getdents64, ___syscall_getpeername, ___syscall_getsockname, ___syscall_getsockopt, ___syscall_ioctl, ___syscall_listen, ___syscall_lstat64, ___syscall_mkdirat, ___syscall_newfstatat, ___syscall_openat, ___syscall_pipe, ___syscall_poll, ___syscall_readlinkat, ___syscall_recvfrom, ___syscall_recvmsg, ___syscall_renameat, ___syscall_rmdir, ___syscall_sendmsg, ___syscall_sendto, ___syscall_socket, ___syscall_stat64, ___syscall_statfs64, ___syscall_symlink, ___syscall_truncate64, ___syscall_unlinkat, ___syscall_utimensat, __mmap_js, __munmap_js, _emscripten_exit_fullscreen, getCanvasSizeMainThread, setCanvasElementSizeMainThread, _emscripten_exit_pointerlock, _emscripten_get_fullscreen_status, _emscripten_get_gamepad_status, _emscripten_get_num_gamepads, _emscripten_request_fullscreen, _emscripten_request_pointerlock, _emscripten_sample_gamepad_data, _emscripten_set_blur_callback_on_thread, _emscripten_set_focus_callback_on_thread, _emscripten_set_fullscreenchange_callback_on_thread, _emscripten_set_gamepadconnected_callback_on_thread, _emscripten_set_gamepaddisconnected_callback_on_thread, _emscripten_set_keydown_callback_on_thread, _emscripten_set_keypress_callback_on_thread, _emscripten_set_keyup_callback_on_thread, _emscripten_set_mousedown_callback_on_thread, _emscripten_set_mousemove_callback_on_thread, _emscripten_set_mouseup_callback_on_thread, _emscripten_set_pointerlockchange_callback_on_thread, _emscripten_set_touchcancel_callback_on_thread, _emscripten_set_touchend_callback_on_thread, _emscripten_set_touchmove_callback_on_thread, _emscripten_set_touchstart_callback_on_thread, _emscripten_set_wheel_callback_on_thread, _environ_get, _environ_sizes_get, _fd_close, _fd_fdstat_get, _fd_read, _fd_seek, _fd_write, _getaddrinfo, _gethostbyaddr, _gethostbyname ];
...
var wasmImports = {
 ...
 "JS_OnScreenshotProgress": _JS_OnScreenshotProgress,
 "JS_OnScreenshotReady": _JS_OnScreenshotReady,
