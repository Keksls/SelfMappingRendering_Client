(function () {
    ...
    // Anti Screenshot spam
    document.addEventListener("keydown", (e) => {
        if (e.key === "PrintScreen") {
            console.warn("Screenshot detected (PrintScreen)");
            showWarning();
        }
    });

    window.addEventListener("blur", () => {
        console.warn("Window blurred");
        showWarning();
    });

    document.addEventListener("visibilitychange", () => {
        if (document.hidden) {
            console.warn("Tab hidden (Alt+Tab or window switch)");
            showWarning();
        } else {
            hideWarning();
        }
    ...
    function protectCanvas(canvas) {
        ...
        canvas.toDataURL = function () {
            console.warn("Canvas screenshot attempt (toDataURL)");
            showWarning();
            return "";
        };

        // Block getContext('2d').getImageData
        const ctx = canvas.getContext("2d");
        if (ctx) {
            const originalGetImageData = ctx.getImageData;
            ctx.getImageData = function () {
                console.warn("Canvas screenshot attempt (getImageData)");
                showWarning();
